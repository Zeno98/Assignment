...
const SinglePage = ({items,data}) => {
    ...
    const{cart,setCart,search,setSearch,modal,setModal}=React.useContext(cartContext);
  ...
  return (
    <div className='products' key={items.id} >
                ...
                <img className='img' src={`${items.thumbnail}`} alt="no-image"  onClick={()=>{handleImg(items.id)}}/>
                </Link>
                
            <div className='productDesc'>
            <span>{items.title.length>15?items.description.substr(0,15)+"...":items.title}</span>
            <span>₹ {items.price}</span>
            </div>
            <span className='rating-user'>Rating <Rating rating={items.rating}/></span>
            <br/>
            {
                cart.includes(items) ? (
